...
function initSmartNearbyExplorer() {
  ...
  const cachePrefix = `${pageConfig.key || cityName || "station"}-smart-nearby-v10`;
...
async function loadPlaces(categoryKey, subcategoryKey) {
  ...
  try {
    ...
    const places = await fetchPlacesFallback(categoryKey, subcategoryKey, subConfig);
    if (places.length) {
  ...
  async function fetchPlacesFallback(categoryKey, subcategoryKey, subConfig) {
    const defaultQueries = {
      ...
      primary: [`általános iskola ${cityName}`],
